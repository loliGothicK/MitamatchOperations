...
public sealed partial class BuilderPage : Page
{
    ...
    private List<MemoriaWithConcentration> selectedMemorias = [];
    ...
    private readonly Lazy<OperationHistory> history = new(() => new());
    ...
    {
        if (e.Parameter is not Edit parameter) return;
        switch (parameter)
        {
            case Undo:
                {
    ...
    {
        selectedMemorias = e.Items.Select(v => ((MemoriaWithConcentration)v)).ToList();
    ...
    {
        foreach (var memoria in selectedMemorias.Where(m => !m.Memoria.Labels.Contains(Label.Legendary)))
        {
        ...
        Pool.RemoveWhere(m => selectedMemorias.Select(s => s.Memoria.Name).Contains(m.Memoria.Name));
        history.Value.Push(new AddMemoria([.. selectedMemorias.Select(m => new MemoriaIdAndConcentration(m.Memoria.Id, m.Concentration))]));
        Cleanup();
    }
    ...
    private void MemeriaSources_Drop(object sender, DragEventArgs e)
    {
        ...
        foreach (var toAdd in OriginalPool
            ...
            .Where(m => selectedMemorias.Select(s => s.Memoria.Name).Contains(m.Memoria.Name)))
        {
            Pool.Add(toAdd);
        }
        foreach (var toRemove in LegendaryDeck.ToList().Where(m => selectedMemorias.Select(m => m.Memoria.Name).Contains(m.Memoria.Name)))
        {
            LegendaryDeck.Remove(toRemove);
        }
        foreach (var toRemove in Deck.ToList().Where(m => selectedMemorias.Select(m => m.Memoria.Name).Contains(m.Memoria.Name)))
        {
            Deck.Remove(toRemove);
        }
        history.Value.Push(new RemoveMemoria([.. selectedMemorias.Select(m => new MemoriaIdAndConcentration(m.Memoria.Id, m.Concentration))]));
        Cleanup();
        Sort(SortOption.SelectedIndex);
    }
    ...
    private async void LoadButton_Click(object _, RoutedEventArgs _e)
    {
        ...
        var oldUnit = new Unit(name, Switch.IsOn, [.. Deck, .. LegendaryDeck]);
        history.Value.Push(new Load(oldUnit, unit));
        LegendaryDeck.Clear();
        Deck.Clear();
    ...
    private void Concentration_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        foreach (var item in LegendaryDeck.ToList())
        {
            ...
            {
                history.Value.Push(new ChangeConcentration(id, item.Concentration, comboBox.SelectedIndex));
